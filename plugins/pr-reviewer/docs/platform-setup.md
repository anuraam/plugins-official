...
| `PR_REVIEWER_MODEL` | unset | **Override:** pins *every* reviewer sub-agent to one model (e.g. `haiku`), ignoring the tiers below. Use it to force a single model for the whole review. Use the short slug (`sonnet`/`opus`/`haiku`/`fable`) — a dated model id like `claude-haiku-4-5` fails validation on harnesses whose `Agent`/`Task` tool restricts `model` to the short-slug enum. |
| `PR_REVIEWER_QUALITY_MODEL` | `haiku` | Model for the **quality-tier** reviewers (`code-reviewer`, `test-reviewer`) on the escalated specialist path — pattern/coverage tasks a small model handles well. Ignored if `PR_REVIEWER_MODEL` is set. |
