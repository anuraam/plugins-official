...
2. Launch the **security-reviewer** agent with `"subagent_type": "security-reviewer"` — **omit** the `model` field so it inherits the lead's model. Pass `/tmp/pr_full_diff_numbered.patch` and `/tmp/pr_changed_files.txt` (inline the numbered diff only if ≤ 300 lines). Include the line-number constraint from `commands/pr-review.md` Step 6.
