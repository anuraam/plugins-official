...
# PreToolUse hook for the Architecture Fitness plugin.
#
# Reading  — git for diffs / file lists; gh / curl for issues, PRs, comments.
# Writing  — the agent pushes only to an `arch/docs-*` branch.
...
# GitHub CLI — used for issue/PR read, docs PR creation, and comment posting
...
if ! git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
    echo '{"decision": "block", "reason": "Not inside a git repository. The Architecture Fitness plugin requires a git project."}'
    exit 0
fi

# Block pushes from any branch that is not an `arch/docs-*` branch.
if echo "$COMMAND" | grep -qE "^git push"; then
    CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "")

    if [ -n "$CURRENT_BRANCH" ] && ! echo "$CURRENT_BRANCH" | grep -qE "^arch/docs-"; then
        echo "{\"decision\": \"block\", \"reason\": \"Refusing to push from '${CURRENT_BRANCH}'. The Architecture Fitness plugin only pushes from branches named 'arch/docs-*' created by the arch-doc-curator agent. Never push to the default branch.\"}"
        exit 0
    fi
fi
...
if echo "$COMMAND" | grep -qE "^git push"; then
    ...
    if echo "$REMOTE_URL" | grep -qE "(dev\.azure\.com|visualstudio\.com)"; then
        ...
        # Generic remotes: allow push without injected tokens (may use existing git credentials).
        # Do not block — provider/generic.md handles push failure gracefully.
        exit 0
    fi
fi
